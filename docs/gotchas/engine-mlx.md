...
### A safetensors index is the active checkpoint's byte manifest

Issue #274 reproduced with a Hub repo that held three root weight files. Its `model.safetensors.index.json` mapped all 2,770 tensors to `model.safetensors` at 5,146,801,426 bytes, while two obsolete split files added another 6,829,300,498 bytes. Discovery summed every `*.safetensors` file, the scheduler added its text-model margin, and `--max-resident-mem` evicted the otherwise healthy text model before FLUX loaded. Deleting the unused files appeared to fix loading only because it changed the incorrect bill.

`safetensors_manifest` is now the one root-weight selector for the generic MLX loader, scanned discovery, `probeModelDir`, and the scheduler's MLX preflight. A valid, nonempty `weight_map` selects each unique referenced root-level safetensors file once, plus recognized root MTP sidecars that load independently. Both loading and accounting consume that exact set. `statFile` makes Hugging Face cache symlinks count their target size. The helper does not trust `metadata.total_size`, which can differ from the real referenced file size. Malformed JSON, empty or wrong-type maps, non-string values, unsafe paths, missing or non-file references, allocation failure, and size overflow all fall back to the conservative legacy scan of every root safetensors file. An invalid manifest must never produce a partial undercount.

The loader must honor the same selection. Loading every root file while billing only indexed files under-reserves duplicate checkpoints and leaks overwritten map keys and array handles. Recognized MTP sidecars remain selected even when the trunk index omits them. Guards: `loadWeights honors the checkpoint index instead of loading obsolete root weights`, `indexed root selection preserves recognized sidecars and excludes unrelated weights`, `discovery and probe bill only safetensors selected by the checkpoint index`, `checkpoint byte accounting falls back when an index cannot be trusted`, and the scheduler `modelDiskBytes` symlink test.
